# first-install-manifest
    ...
    **Live infrastructure:** None for deterministic pull-request checks.
    ...
    1. Verify durable first-install manifest creation without damaging native manifests across the supported happy-path states and canonical fixtures.
2. Verify durable first-install manifest creation without damaging native manifests under retries, interruption, concurrency, offline operation, or partial failure.
3. Verify durable first-install manifest creation without damaging native manifests preserves authorization, idempotency, integrity, observability, and actionable failure classification.
